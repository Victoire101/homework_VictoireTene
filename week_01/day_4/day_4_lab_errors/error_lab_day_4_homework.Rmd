...
#sample_n (animals, 10, replace = TRUE) # must be a dataframe for sample_n
...
scent_ratings %>% janitor::clean_names()
 ...
 iris_mean <- scent_ratings %>% 
  drop_na(rating) %>% 
  group_by(SCENT) %>% 
  summarise(mean_rating = mean(rating)) %>% 
summarise( total_mean = mean (mean_rating))  
 # pull(total_mean)
